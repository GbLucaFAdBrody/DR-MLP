...
class ActivateNet:
    ...
    # prediction_feature是要预测的特征名，epochs是训练周期的次数，ints_only是一个布尔值，用于指定是否仅使用整数和一些特殊字符进行训练
        ...
        # 如果ints_only为假，表示使用所有可打印字符进行数据处理
            ...
            #  创建一个GeneralFormat实例用于数据的格式化处理，指定为非整数模式。
    ...
    def heatmap_weights(self, index):
        ...
        plt.savefig('DeepResidualMLP_heatmap_weight_{0:04d}.png'.format(index), dpi=400, bbox_inches='tight', pad_inches=0)
        plt.close()
        return
    ...
    def heatmap_biases(self, index):
        ...
        # plt.style.use('dark_background')
        plt.savefig('DeepResidualMLP_heatmap_biases_{0:04d}.png'.format(index), dpi=400, bbox_inches='tight', pad_inches=0)
        plt.close()
    ...
    def plot_embedding(self, index=0):
        ...
        for k in range(0, 1):
            ...
            plt.savefig('DeepResidualMLP_input_embedding_residual/DeepResidualMLP_input_embedding_residual.png'.format(index), dpi=390)
            plt.close()
        return
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(input_tensors) - minibatch_size, minibatch_size):
                ...
                if count % 100 == 0:
                    ...
                    self.plot_predictions(count // 100)  # count//100
...
if __name__ == '__main__':
    network = ActivateNet('positive_three', 200)# 800
    network.train_model()
    # hidden
