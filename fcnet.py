...
class ActivateNet:
	...
	def plot_predictions(self, epoch_number):
		...
		plt.axis([-10,140,-10,120]) # x-axis range followed by y-axis range
		# [-10, 150, -10, 120]
		# plt.show()
	...
	def plot_embedding(self, index=0):
		...
		plt.ylabel('Embedding Distance', fontsize=8)
	...
	def train_model(self, minibatch_size=128):
		...
		plt.savefig('loss_fcnet/loss_fcnet.png', dpi=400)
		plt.legend()
		plt.show()
		return
...
# 使用occlusion方法：通过遮蔽特定的输入特征来观察对模型预测的影响，然后生成解释性的条形图和热图。这种方法尤其适用于理解哪些特征在模型做出决策时是重要的，因为它通过模拟去除特征来观察模型性能的变化。
interpret.readable_interpretation(0, method='occlusion', aggregation='max')
interpret.heatmap(0, method='occlusion')
# 使用gradientxinput方法：通过计算每个输入特征的梯度并将其乘以输入值来评估特征的重要性，然后同样生成解释性的条形图和热图。这适用于理解模型预测对每个输入特征变化的敏感度。
interpret.readable_interpretation(1, method='gradientxinput', aggregation='max')
interpret.heatmap(1, method='gradientxinput')
# 使用combined方法：结合上述两种方法的优点，提供一个综合视角来评估特征的重要性，并通过条形图和热图展示。
interpret.readable_interpretation(2, method='combined', aggregation='max')
interpret.heatmap(2, method='combined')
interpret = StaticInterpret(model, input_tensors, output_tensors)
# 使用occlusion方法：通过遮蔽特定的输入特征来观察对模型预测的影响，然后生成解释性的条形图和热图。这种方法尤其适用于理解哪些特征在模型做出决策时是重要的，因为它通过模拟去除特征来观察模型性能的变化。
interpret.readable_interpretation(3, method='occlusion', aggregation='average')
interpret.heatmap(3, method='occlusion')
# 使用gradientxinput方法：通过计算每个输入特征的梯度并将其乘以输入值来评估特征的重要性，然后同样生成解释性的条形图和热图。这适用于理解模型预测对每个输入特征变化的敏感度。
interpret.readable_interpretation(4, method='gradientxinput', aggregation='average')
interpret.heatmap(4, method='gradientxinput')
# 使用combined方法：结合上述两种方法的优点，提供一个综合视角来评估特征的重要性，并通过条形图和热图展示。
interpret.readable_interpretation(5, method='combined', aggregation='average')
interpret.heatmap(5, method='combined')
"""
...
model = network.model
network.count_parameters(model)
interpret = StaticInterpret(model, input_tensors, output_tensors,embedding_dim)
# 使用occlusion方法：通过遮蔽特定的输入特征来观察对模型预测的影响，然后生成解释性的条形图和热图。这种方法尤其适用于理解哪些特征在模型做出决策时是重要的，因为它通过模拟去除特征来观察模型性能的变化。
interpret.readable_interpretation(0, method='occlusion', aggregation='max')
interpret.heatmap(0, method='occlusion')
# 使用gradientxinput方法：通过计算每个输入特征的梯度并将其乘以输入值来评估特征的重要性，然后同样生成解释性的条形图和热图。这适用于理解模型预测对每个输入特征变化的敏感度。
...
# interpret.heatmap(1)
# interpret.readable_interpretation(2, method='max')
# interpret.heatmap(2)
